                ...
                if buddies.contacts.isEmpty {
                    ...
                    ForEach($buddies.contacts) { b in
                        VStack(alignment: .leading, spacing: 6) {
                            ...
                            Picker("Delivery", selection: Binding(get: { b.mode.wrappedValue ?? .askFirst }, set: { b.mode.wrappedValue = $0 })) {
                                Text("Auto-send").tag(BuddyContact.DeliveryMode.autoSend)
                                Text("Ask first").tag(BuddyContact.DeliveryMode.askFirst)
                            }
                            .pickerStyle(.segmented)
                            .onAppear {
                                if b.mode.wrappedValue == nil {
                                    b.mode.wrappedValue = .askFirst
                                }
                            }
                        }
                    }
...
struct ContactPickerView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .onAppear {
                loadContacts()
                PostHogService.shared.screen("Buddy System")
            }
        }
    }
