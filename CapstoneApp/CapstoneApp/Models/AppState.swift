...
////
//// C++/Systems mental model:
////   - `ObservableObject`  ~ a ref-counted subject that emits "state changed"
////     signals (think: a thread-constrained, UI-facing event/source object).
////   - `@Published`        ~ lightweight signal/slot; writes publish change
////     notifications to the SwiftUI runtime (like notifying observers).
////   - `UserDefaults`      ~ small, process-local key/value store (INI/registry-ish).
////   - `enum Mode: String` ~ string-backed state machine mode (stable ABI for persistence).
////   - ARC                 ~ deterministic ref counting (like shared_ptr). Watch closure
////     captures in `BottomNotice.action` to avoid retain cycles.
////
//// Concurrency:
////   UI/state writes should occur on the main thread. Consider `@MainActor` on AppState
////   if you mutate it from async contexts to enforce main-thread updates.
//// -----------------------------------------------------------------------------
...
final class AppState: ObservableObject {
    ...
    @Published var mode: Mode = .idle {
        didSet {
            UserDefaults.standard.set(mode.rawValue, forKey: "SB.mode")
            // Update Dynamic Island Live Activity
            switch mode {
            case .detecting:
                ModeActivityManager.shared.startActivity(mode: .detect)
            case .sleeping:
                ModeActivityManager.shared.startActivity(mode: .sleep)
            default:
                ModeActivityManager.shared.endActivity()
            }
        }
    }
    ...
    enum Tab { case home, incidents }
    @Published var tab: Tab = .home {
        didSet {
            if tab == .incidents { 
                hasUnseenIncident = false
                // Clear the notification badge on the app icon
